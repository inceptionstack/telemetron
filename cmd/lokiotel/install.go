...
func newInstallCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if runtime.GOOS == "darwin" {
				fmt.Fprintln(cmd.ErrOrStderr(), "lokiotel install is not supported on macOS.")
				fmt.Fprintln(cmd.ErrOrStderr(), "You can still run `lokiotel start --config /path/to/config.yaml` directly or under launchd. See docs/macos.md.")
	...
	cmd.Flags().StringVar(&endpoint, "endpoint", "", "OTLP endpoint (env: LOKIOTEL_ENDPOINT, config: endpoint)")
	cmd.Flags().StringVar(&token, "token", "", "bearer token (env: LOKIOTEL_TOKEN, written to token_file)")
	cmd.Flags().StringVar(&mode, "mode", "", "collection mode (env: LOKIOTEL_MODE, config: mode)")

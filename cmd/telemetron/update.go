...
applying.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Resolve which binary to update: prefer managed path, fall back
			// to legacy for pre-migration installs that only have /usr/local/bin.
			binaryPath := updater.ManagedBinaryPath
			if _, err := os.Stat(updater.ManagedBinaryPath); err != nil {
				binaryPath = updater.LegacyBinaryPath
			}
			up := updater.NewForManualUpdate(version, binaryPath)
